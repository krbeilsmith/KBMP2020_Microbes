...
# Frequency plots
freq <- ggplot(Diversity, aes(log10(SampRelAbund),stat(density))) + geom_histogram(size=1,binwidth=0.1,fill="gray70",color="gray70")+
  facet_grid(rows=vars(Stage),switch="y",labeller=labeller(Stage=Stage.labs))+
  ...
  font("xy.text", size = 12, color = "black", face = "bold")+ 
  font("xlab", size = 16, color = "black",face = "bold")+ 
  labs(x="\nlog10 ASV Relative Abundances",y="Frequency in Plant Samples\n")+
  theme(strip.text.y = element_text(size=12, face="bold",margin = margin(0,0,0,1, "cm"),vjust=5))+
  theme(strip.placement = "outside")+
  theme(panel.spacing = unit(1, "lines"))+
  geom_vline(data=medians, aes(xintercept=log10(grp.median)),linetype=1, color="black",size=1)+
  geom_vline(data=Diversity, aes(xintercept=log10(0.1)),linetype="dashed", color="black",size=0.5)+
  rremove("legend")

ggsave("~/Documents/KBMP2020_Microbes/Figures/RelativeAbundancesByStage.tiff", plot = freq, device = NULL, path = NULL,
       scale = 1, width = 5, height = 10, units = c("in"),
       dpi = 600, limitsize = TRUE)
