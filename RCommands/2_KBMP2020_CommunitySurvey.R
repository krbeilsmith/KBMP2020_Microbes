...
CommunitySurvey <- NULL
CommunitySurvey <- data.frame(matrix(ncol = 12, nrow = 0),stringsAsFactors = FALSE)
CommunitySurvey  <- setNames(data.frame(CommunitySurvey), c("ASV","Tissue","Stage","Site","Year","RelAbund","Abund",
...
for(m in taxa_names(AbundantTaxa)){ # for each ASV 
  ...
  for(row in rownames(ConditionsFrame)){ # for each condition
    ...
    CommunitySurvey[nrow(CommunitySurvey)+1,] <- c(m,pp,st,si,yr,RelAbund,Abund,Genus,Family,Order,Class,Phylum) # store data on this ASV in the frame
  }
}

# Save dataframe
write.table(CommunitySurvey,file="~/Documents/KBMP2020_Microbes/Outputs/CommunitySurvey", sep="\t", row.names=TRUE,quote=FALSE)

##########################################################################################################################################################

# Load data for plotting
CommunitySurvey <- read.table("~/Documents/KBMP2020_Microbes/Outputs/CommunitySurvey", header=TRUE, sep="\t",row.names=1)

# Set factor levels for plotting.
CommunitySurvey$Tissue <-factor(CommunitySurvey$Tissue, levels=c("Soil","Roots","RosLeaves","Stems","CauLeaves","Flowers","Siliques"))
CommunitySurvey$Stage <-factor(CommunitySurvey$Stage, levels=c("Soil","TwoLeaf","FourLeaf","SixLeaf","EightLeaf","Flowering","Senescent"))
CommunitySurvey$Site <-factor(CommunitySurvey$Site, levels=c("ME","WW"))
CommunitySurvey$Year <-factor(CommunitySurvey$Year, levels=c("1","2"))
CommunitySurvey$Genus <-factor(CommunitySurvey$Genus)
CommunitySurvey$Family <-factor(CommunitySurvey$Family)
CommunitySurvey$Order <-factor(CommunitySurvey$Order)
CommunitySurvey$Class <-factor(CommunitySurvey$Class)
CommunitySurvey$Phylum <-factor(CommunitySurvey$Phylum)

# Set values to numeric for plotting.
CommunitySurvey$RelAbund <- as.numeric(CommunitySurvey$RelAbund)
CommunitySurvey$Abund <- as.numeric(CommunitySurvey$Abund)

# Assign samples as either Soil or Plant tissue for plotting.
CommunitySurvey$IsSoil <- ifelse(CommunitySurvey$Tissue=="Soil","Soil","Plant")
CommunitySurvey$IsSoil <- factor(CommunitySurvey$IsSoil,levels=c("Soil","Plant"))

# Rename the "unclassified" class category. 
CommunitySurvey$Class <- ifelse(CommunitySurvey$Class=="aggregate unclassified class","unclassified",as.character(CommunitySurvey$Class))

# check data
# CommunitySurvey[CommunitySurvey$Class=="Bacilli" & CommunitySurvey$Present>0,]
...
CS1 <- CommunitySurvey %>% filter(Year=="2") %>% filter(Class!="unclassified") %>% filter(Tissue%in%c("Soil","Roots")) %>%
...
# Plot
ggsave("~/Documents/KBMP2020_Microbes/Figures/CommunitySurveyA.png", plot = grid.arrange(
  CommunitySurvey1,
  layout_matrix = rbind(c(1))), device = NULL, path = NULL,
  scale = 2, width = 3, height = 3, units = c("in"),
  dpi = 600, limitsize = TRUE)

# stat test results in dataframe
# compare_means(data=CommunitySurvey %>% filter(Class!="unclassified") %>% filter(Year=="2"),formula= RelAbund~IsSoil,method = "kruskal.test",group.by=c("Class"))
...
CS2 <- CommunitySurvey %>% filter(Year=="2") %>% filter(Class!="unclassified") %>% filter(Tissue!="Soil") %>%
...
# Plot
ggsave("~/Documents/KBMP2020_Microbes/Figures/CommunitySurveyB.png", plot = grid.arrange(
  CommunitySurvey2,
  layout_matrix = rbind(c(1))), device = NULL, path = NULL,
  scale = 2, width = 3, height = 3, units = c("in"),
  dpi = 600, limitsize = TRUE)
...
# Plot together
ggsave("~/Documents/KBMP2020_Microbes/Figures/CommunitySurvey.png", plot = grid.arrange(
  CommunitySurvey1+ggtitle("A\n")+theme(plot.title = element_text(face="bold",size=20)),
  CommunitySurvey2+ggtitle("B\n")+theme(plot.title = element_text(face="bold",size=20)),
  layout_matrix = rbind(c(1,2))), device = NULL, path = NULL,
  scale = 2, width = 6, height = 3, units = c("in"),
